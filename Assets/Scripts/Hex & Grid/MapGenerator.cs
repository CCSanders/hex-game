...
public class MapGenerator : MonoBehaviour {
	...
	[Range(0f, 0.3f)]
	public float mountainProbabilty = 0.05f;
	...
	static float[] moistureBands = { 0.15f, 0.30f, 0.90f };

    //grid of biomes. left -> right = moisture, top -> bottom = temperature. 
    // first field is ground type, second field is amount of plants
	static Biome[] biomes = {
		new Biome(TerrainType.Sand, TerrainType.NoPlants), new Biome(TerrainType.Snow, TerrainType.NoPlants), new Biome(TerrainType.Snow, TerrainType.NoPlants), new Biome(TerrainType.Snow, TerrainType.NoPlants),
		new Biome(TerrainType.Sand, TerrainType.NoPlants), new Biome(TerrainType.Rock, TerrainType.NoPlants), new Biome(TerrainType.Rock, TerrainType.LightPlants), new Biome(TerrainType.Rock, TerrainType.Forest),
		new Biome(TerrainType.Sand, TerrainType.NoPlants), new Biome(TerrainType.Grass, TerrainType.NoPlants), new Biome(TerrainType.Grass, TerrainType.LightPlants), new Biome(TerrainType.Grass, TerrainType.Forest),
		new Biome(TerrainType.Sand, TerrainType.NoPlants), new Biome(TerrainType.Grass, TerrainType.LightPlants), new Biome(TerrainType.Grass, TerrainType.Forest), new Biome(TerrainType.Grass, TerrainType.Jungle)
	};
	...
	int SinkTerrain (int chunkSize, int budget, MapRegion region) {
		...
		int sink = Random.value < sinkProbability ? 1 : 0;
		int size = 0;
		while (size < chunkSize && searchFrontier.Count > 0) {
			HexCell current = searchFrontier.Dequeue();
			int originalElevation = current.Elevation;
		...
		for (int i = 0; i < cellCount; i++) {
			...
			if (!cell.IsUnderwater) {
				...
				if (cell.Elevation == elevationMaximum) {
					cellBiome.terrain = TerrainType.Snow;
                    if (cell.HasRiver)
                    {
                        cell.RemoveRiver();
                    }
                ...
                if(moisture < moistureBands[0])
                {
                    Debug.Log("pretty sure this should be a desert tile.");
                }

                //if snow, remove plants. if has river and not max plants, add a plant
				if (cellBiome.terrain == TerrainType.Snow) {
					cellBiome.plant = 0;
				}
				...
				int terrain;
				if (cell.Elevation == TerrainType.Coast || cell.Elevation == TerrainType.Ocean) {
					int shore = 0;
					for (HexDirection d = HexDirection.NE; d <= HexDirection.NW; d++) {
						HexCell neighbor = cell.GetNeighbor(d);
                    ...
                    else if(shore > 0)
                    {
                        ...
                        cell.Elevation = TerrainType.Coast;
                    }
					else
                    {
						terrain = TerrainType.Dirt;
                        cell.Elevation = TerrainType.Ocean;
					}
				}
				else if (cell.Elevation >= waterLevel) {
                    //one possibility of lake
                    terrain = TerrainType.Grass;
                    cell.IsLake = true;
                }
				else {
                    //shouldn't happen, broken water
                    terrain = TerrainType.Grass;
				}
